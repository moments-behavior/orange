...
static std::vector<uint8_t> build_bringup_reply(const std::string &name,
    ...
    auto srv = CreateServer(b, Kind_KindReply, ServerControl_NONE,
                            ...
                            /*epoch*/ 0,
                            /*seq*/ 0,
                            /*command_body_type*/ CommandBody_NONE,
                            /*command_body*/ 0,
                            /*reply*/ rep);
    b.Finish(srv);
...
build_phase_reply(const camnet::v1::Server *cmd, const std::string &name,
    ...
    auto rep =
        CreateReplyInfo(b, st, ok, code, det, /*bringup*/ 0, cam_count, sid);

    // Explicit type to avoid 'auto' deducing int if absent
...
static void server_on_event(const Incoming &evt) {
    ...
    case Incoming::Receive: {
        ...
        // Idempotent handling: if we already reached/advanced past this control
        if (ctrl <= g_last_done) {
        ...
        // COMM ONLY: pretend success immediately
        auto st = state_after(ctrl);
...
int main(int argc, char **argv) {
    ...
    g_name = (argc > 1) ? argv[1] : "camA";
    int port = (argc > 2) ? std::atoi(argv[2]) : 34001;

    if (!ctx.net.start_server(port)) {
    ...
    for (;;) {
        ctx.net.step(10, server_on_event);
