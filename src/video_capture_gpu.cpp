...
void aquire_frames_gpu_encode(Emergent::CEmergentCamera* camera, Emergent::CEmergentFrame* frame_recv, int num_frames)
{
    int camera_return {0};
    ...
    for(int frame_count=0;frame_count<num_frames;frame_count++)
    {
        ...
        if(!camera_return)
        {
            ...
            {
                frames_recd++;
                // upload to gpu, can consider do this in a different thread
