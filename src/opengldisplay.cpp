    ...
    while(IsMachineOn())
    {
        ...
        if(f) {
            ...
            if (camera_select->frame_save_state==State_Write_New_Frame) {
                ...
                std::string image_name = camera_select->picture_save_folder + "/Cam" + camera_params->camera_serial + "_" + camera_select->frame_save_name + "." + camera_select->frame_save_format;
