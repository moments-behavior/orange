        ...
        if (ImGui::Begin("Orange Streaming", NULL, ImGuiWindowFlags_MenuBar))
        {
            ...
            if (ImGui::Button(camera_control->subscribe ? "Stop streaming" : "Start streaming"))
            {
                ...
                {   
                    ...
                    for (int i = 0; i < num_cameras; i++)
                    {
                        // not sure where the bug is for memory issue
                        // if (cameras_select[i].stream_on) {
                    ...
                    for (int i = 0; i < num_cameras; i++)
                    {
                        detection_threads.push_back(std::thread(&detection_proc, sync_display, cameras_params, camera_control, &cameras_select[i], tex[i].cuda_buffer, detection_data, i));
                    }
