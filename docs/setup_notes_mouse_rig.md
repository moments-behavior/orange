...
- [cudnn (depends on CUDA installation)](#install-cudnn-depends-on-cuda-installation)
- [tensor-rt (depends on nvidia-driver and CUDA)](#install-tensor-rt)
- [ffmpeg](#install-ffmpeg)
- [orange](#install-orange) 
...
**1. download and build ffnvcodec**
  - install this prerequisite as:
    ```
    cd /home/$USER/nvidia
...
**2. download ffmpeg source code**
  - clone and checkout:
    ...
    ```
**3. specify gpu architecture**
  -  open the configure script: 
      ```
      vim configure 
      ```
  - set the `nvccflags_default` instancse to use more recent gpu architecture, `compute_75`
    ```
...
**4. configure, compile, and install ffmpeg with NVIDIA GPU support**:
  - configure:
    ```
    cd ~/nvidia/ffmpeg/  
...
### install orange 
  - see `orange` [repository](https://github.com/JohnsonLabJanelia/orange/blob/unify/README.md)