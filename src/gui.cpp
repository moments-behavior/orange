...
void start_camera_streaming(
    ...
    if (ptp_stream_sync) {
        // Cross-camera start gate only makes sense with >1 camera, or when a
        // networked session must align this host with others. A stand-alone
        // single camera enables PTP (for timestamps) but free-runs — no
        // gate/countdown.
        bool gated_start = num_cameras > 1 || network_sync_requested;
        for (int i = 0; i < num_cameras; i++) {
