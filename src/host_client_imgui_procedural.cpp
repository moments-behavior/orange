...
static void OnOpenPhaseStart() {
    ...
    open_selected_cameras(check, cam_count, devs, cfg_files,
                          num_cams, params, select, ecams, plots);
    if (ctrl) ctrl->open = true;
}
...
static std::vector<uint8_t> build_cmd_open(const std::string &job_id,
    ...
    auto msg  = CreateServer(b, Kind_KindCommand, ServerControl_OPENCAMERA,
                             jid, epoch, seq, CommandBody_OpenArgs, args.Union(), 0);
    b.Finish(msg);
...
build_cmd_startthreads(const std::string &job_id, uint32_t epoch, uint32_t seq,
    ...
    auto msg  = CreateServer(b, Kind_KindCommand, ServerControl_STARTTHREAD,
                             jid, epoch, seq, CommandBody_StartThreadsArgs, args.Union(), 0);
    b.Finish(msg);
    return { b.GetBufferPointer(), b.GetBufferPointer() + b.GetSize() };
}
...
static std::vector<uint8_t> build_cmd_start(const std::string &job_id,
    ...
    auto msg  = CreateServer(b, Kind_KindCommand, ServerControl_STARTRECORDING,
                             jid, epoch, seq, CommandBody_StartArgs, args.Union(), 0);
    b.Finish(msg);
    return { b.GetBufferPointer(), b.GetBufferPointer() + b.GetSize() };
}
...
static std::vector<uint8_t> build_cmd_stop(const std::string &job_id,
    ...
    auto msg  = CreateServer(b, Kind_KindCommand, ServerControl_STOPRECORDING,
                             jid, epoch, seq, CommandBody_StopArgs, args.Union(), 0);
    b.Finish(msg);
    return { b.GetBufferPointer(), b.GetBufferPointer() + b.GetSize() };
}
...
// We default to NOT stepping ENet in the GUI tick; a dispatcher thread handles it.
...
static camnet::v1::ServerControl current_ctrl() {
    switch (g_phase) {
    case Phase_Open:    return camnet::v1::ServerControl_OPENCAMERA;
    case Phase_Threads: return camnet::v1::ServerControl_STARTTHREAD;
    case Phase_Start:   return camnet::v1::ServerControl_STARTRECORDING;
    case Phase_Stop:    return camnet::v1::ServerControl_STOPRECORDING;
    default:            return camnet::v1::ServerControl_NONE;
    }
}

static const char *phase_name() {
    switch (g_phase) {
    case Phase_Open:    return "OPENCAMERA";
    case Phase_Threads: return "STARTTHREAD";
    case Phase_Start:   return "STARTRECORDING";
    case Phase_Stop:    return "STOPRECORDING";
...
static void advance_phase() {
    switch (g_phase) {
    case Phase_Open:    g_phase = Phase_Threads; break;
    case Phase_Threads: g_phase = Phase_Start;   break;
    case Phase_Start:   g_phase = Phase_Stop;    break;
    case Phase_Stop:    g_phase = Phase_Done;    break;
    default: break;
    }
}
...
static void broadcast_current_phase() {
    ...
    case Phase_Open:
        ...
        break;
    case Phase_Threads:
        bytes = build_cmd_startthreads(g_jid, g_epoch, g_seq, "out/session-1", "h264");
        break;
    case Phase_Start:
...
static void host_on_event(const Incoming &evt) {
    ...
    case Incoming::Receive: {
        ...
        if (!parse_server(evt.bytes, msg)) break;
        if (msg->kind() != camnet::v1::Kind_KindReply) break;
        auto rep = msg->reply();
        if (!rep) break;
        ...
        if (auto br = rep->bringup()) {
            ...
            g_ctxp->peers.set_bringup(evt.peer_id, name, br->num_cameras(), rep->state());
            logf("bringup from %s cams=%d state=%d (pid=%u)",
                 name.c_str(), br->num_cameras(), (int)rep->state(), evt.peer_id);
        }
...
void HostClient_StartNetThread(AppContext &ctx) {
    if (g_net_run.exchange(true)) return;
    g_net_thr = std::thread(HostClient_NetThreadMain, &ctx);
}

void HostClient_StopNetThread() {
    if (!g_net_run.exchange(false)) return;
    if (g_net_thr.joinable()) g_net_thr.join();
}
...
void HostClient_Tick() {
    ...
    if (!batch.empty()) {
        for (const ReplyEvent &e : batch) {
            if (e.job_id != g_jid || e.epoch != g_epoch || e.seq != g_seq) continue;
            if (e.ctrl   != current_ctrl()) continue;
            logf("%s reply from %s ok=%d state=%d",
                 ctrl_name(e.ctrl), e.server_id.c_str(), (int)e.ok, (int)e.state);
            if (e.ok) g_ack_by[e.server_id] = true;
        }

        if (g_waiting) {
            bool all = true;
            for (const auto &name : g_servers) {
                if (!g_ack_by.count(name) || !g_ack_by[name]) { all = false; break; }
            }
            if (all) {
        ...
        if (now - g_last_send > std::chrono::milliseconds(g_timeout_ms)) {
            for (const auto& name : g_servers) {
                if (!g_ack_by.count(name) || !g_ack_by[name]) {
                    logf("waiting for %s on %s …", name.c_str(), ctrl_name(current_ctrl()));
                }
            }
...
void HostClient_DrawImGui() {
    ...
    if (ImGui::CollapsingHeader("Cameras", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        for (const auto &pi : info) {
            ImGui::BulletText("pid=%u name=%s camsK=%d cams=%d stateK=%d state=%d",
