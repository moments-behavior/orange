...
__device__ void get_class_color(int class_id, unsigned char& r, unsigned char& g, unsigned char& b) {
    // helper for picking bounding box colors
...
__global__ void gpu_draw_box(unsigned char* src, const int width, const int height, float* d_points, int label_id, double current_time)
{
    // override to add class-based box colors
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void gpu_draw_box(unsigned char* src, int width, int height, float* d_points, int label_id, cudaStream_t stream)
{
    ...
    dim3 num_blocks((width + threads_per_block.x -1) / threads_per_block.x, (height + threads_per_block.y -1) / threads_per_block.y);
    double current_time = (double) (std::chrono::system_clock::now().time_since_epoch()).count();
    gpu_draw_box <<<num_blocks, threads_per_block, 0, stream>>> (src, width, height, d_points, label_id, current_time);
}

__global__ void gpu_draw_obb_kernel(unsigned char* src, const int width, const int height, 
