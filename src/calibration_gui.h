...
void calibration_window(CPURender* cpu_buffers, Settings* calib_setting, CameraControl *camera_control, CameraParams *cameras_params, u32 num_cameras, CameraCalibResults* calib_results, CalibData* calib_data) 
{
    if (ImGui::Begin("Calibration"))
    {
        ...
        if (camera_control->calibration) {
            ...
            {
                for(int i=0; i < num_cameras; i++){
                    cpu_buffers->display_buffer[i].available_to_write = false;
                }
                
                for(int i=0; i < num_cameras; i++) {
                    cv::Mat view = cv::Mat(3208 * 2200 * 3, 1, CV_8U, cpu_buffers->display_buffer[i].frame).reshape(3, 2200);
                ...
                for(int i=0; i < num_cameras; i++){
                    cpu_buffers->display_buffer[i].available_to_write = true;
                }
            }
            ...
            for(int j=0; j<num_cameras; j++){
                ...
                bind_texture(&cpu_buffers->image_texture[j]);
                glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 3208, 2200, GL_RGB, GL_UNSIGNED_BYTE, cpu_buffers->display_buffer[j].frame);
                unbind_texture();
            }
            ...
            for (int i = 0; i < num_cameras; i++) {
                ...
                if (ImPlot::BeginPlot("##no_plot_name", avail_size)){
                    ImPlot::PlotImage("##no_image_name", (void*)(intptr_t)cpu_buffers->image_texture[i], ImVec2(0,0), ImVec2(cameras_params[i].width, cameras_params[i].height));
                    ImPlot::EndPlot();
                }
