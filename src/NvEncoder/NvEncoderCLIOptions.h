...
class NvEncoderInitParam {
    ...
    NvEncoderInitParam(const char *szParam = "",
        ...
        for (unsigned i = 0; i < tokens.size(); i++) {
            ...
            if (tokens[i] == "-tuninginfo" && ++i != tokens.size()) {
                ParseString("-tuninginfo", tokens[i], vTuningInfo,
                            szTuningInfoNames, &m_TuningInfo);
                continue;
            }
        }
    }
    ...
    std::string GetHelpMessage(bool bMeOnly = false, bool bUnbuffered = false,
        ...
        if (!bMeOnly) {
            if (bLowLatency == false)
                oss << "-tuninginfo  TuningInfo: " << szTuningInfoNames
                    << std::endl;
            else
                oss << "-tuninginfo  TuningInfo: "
                    << szLowLatencyTuningInfoNames << std::endl;
        ...
        if (!bLowLatency) {
            oss << "-rc          Rate control mode: " << szRcModeNames
                << std::endl
                ...
                << std::endl
                << "-vbvbufsize  VBV buffer size in bits, can be in unit of 1, "
                   "K, M"
                << std::endl
                << "-vbvinit     VBV initial delay in bits, can be in unit of "
                   "1, K, M"
                << std::endl
                << "-aq          Enable spatial AQ and set its stength (range "
                   "1-15, 0-auto)"
                << std::endl
                << "-temporalaq  (No value) Enable temporal AQ" << std::endl
                << "-cq          Target constant quality level for VBR mode "
                   "(range 1-51, 0-auto)"
                << std::endl;
        }
        if (!bUnbuffered && !bLowLatency) {
            oss << "-lookahead   Maximum depth of lookahead (range 0-(31 - "
                   "number of B frames))"
                << std::endl;
        }
        ...
        for (unsigned i = 0; i < tokens.size(); i++) {
            if (tokens[i] == "-codec" && ++i || tokens[i] == "-preset" && ++i ||
                ...
                tokens[i] == "-multipass" && ++i != tokens.size() &&
                    ParseString("-multipass", tokens[i], vMultiPass,
                                szMultipass, &config.rcParams.multiPass) ||
            ...
            errmessage << "Re-run the application with the -h option to get a "
  ...
  private:
    /*
     * Helper methods for parsing tokens (generated by splitting the command
     * line) and performing conversions to the appropriate target type/value.
     */
    template <typename T>
    ...
    std::string ConvertValueToString(const std::vector<T> &vValue,
        ...
        if (it == vValue.end()) {
            LOG(ERROR) << "Invalid value. Can't convert to one of "
                       << strValueNames;
            return std::string();
        }
    ...
    bool ParseBitRate(const std::string &strName, const std::string &strValue,
        ...
        try {
            ...
            if (c != 0 && c != 'k' && c != 'm') {
                LOG(ERROR) << strName
                           << " units: 1, K, M (lower case also allowed)";
            }
            *pBitRate =
