...
int main(int argc, char *argv[])
{
    ...
    std::string encoder_setup = "-preset p1 -fps " + std::to_string(cameras_params[0].frame_rate);
    ...
    for (int i = 0; i < num_cameras; i++)
    {
        camera_threads.push_back(std::thread(&aquire_frames, &ecams[i], &cameras_params[i], camera_control, display_buffer, encoder_setup, folder_name, ptp_params));
    }
