...
class Dataset2D(BaseDataset):
    ...
    def __init__(self, cfg, set="train", mode="CenterDetect", **kwargs):
        ...
        self.mode = mode
        assert (
            cfg.KEYPOINTDETECT.BOUNDING_BOX_SIZE % 64 == 0
        ...
        if self.mode == "CenterDetect":
            ...
            for output_size in output_sizes:
                self.heatmap_generators.append(
                    HeatmapGenerator(
                        [
            ...
            self.heatmap_generators = []
            output_sizes = [
                int(cfg.KEYPOINTDETECT.BOUNDING_BOX_SIZE / 4),
    ...
    def _build_augpipe(self):
        ...
        if self.mode == "CenterDetect":
            img = self._load_image(0)
            width, height = img.shape[1], img.shape[0]
            scale = 1.0 / self.cfg.CENTERDETECT.IMAGE_SIZE
            self.scale_width = float(width) / self.cfg.CENTERDETECT.IMAGE_SIZE
            self.scale_height = (
                float(height) / self.cfg.CENTERDETECT.IMAGE_SIZE
            )

            augmentors.append(
                iaa.Resize(
                    self.cfg.CENTERDETECT.IMAGE_SIZE, interpolation="linear"
                )
            )
        ...
        if not (self.mode == "CenterDetect" and self.set_name == "val"):
            ...
            augmentors.append(
                iaa.Sometimes(
                    cfg.AFFINE_TRANSFORM.PROBABILITY,
                    iaa.Affine(
                        rotate=cfg.AFFINE_TRANSFORM.ROTATION_RANGE,
    ...
    def _get_item_center(self, idx):
        ...
        animal_size = np.max(
            [bboxs[0][3] - bboxs[0][1], bboxs[0][2] - bboxs[0][0]]
        )
        ...
        for scale_id in range(2):
            target_t = self.heatmap_generators[scale_id](
                joints_list[scale_id], animal_size
            )
    ...
    def _get_item_keypoints(self, idx):
        ...
        center_y = min(
            max(bbox_hw, int((bboxs[0][1] + int(bboxs[0][3])) / 2)),
            img.shape[0] - bbox_hw,
        )
        center_x = min(
            max(bbox_hw, int((bboxs[0][0] + int(bboxs[0][2])) / 2)),
            img.shape[1] - bbox_hw,
        )
        img = img[
        ...
        if self.set_name == "train":
            ...
            img, keypoints_aug = self.augpipe(
        ...
        for i, keypoint in enumerate(keypoints.reshape((-1, 3))):
            if (
                ...
                keypoints_new[:, i * 3 : i * 3 + 2] = keypoints[
                    :, i * 3 : i * 3 + 2
                ]
    ...
    def get_dataset_config(self):
        ...
        # plt.show()
        bbox_min_size = np.max(
            [np.percentile(x_sizes, 98), np.percentile(y_sizes, 98)]
        )
    ...
    def visualize_sample(self, idx):
        ...
        heatmaps = sample[1]
        img = cv2.resize(
            img * 255, (heatmaps[1][0].shape[1], heatmaps[1][0].shape[0])
        ).astype(np.uint8)
        colored_heatmap = cv2.applyColorMap(
            heatmaps[1][0].astype(np.uint8), cv2.COLORMAP_JET
        )
...
class HeatmapGenerator:
    ...
    def __call__(self, joints, size):
        hms = np.zeros(
            (self.num_joints, self.output_res[0], self.output_res[1]),
            dtype=np.float32,
        )
        ...
        for p in joints:
            for idx, pt in enumerate(p):
                if pt[0] != 0 or pt[1] != 0:
                    x, y = (
                        int(pt[0] * self.scale_factor),
                        int(pt[1] * self.scale_factor),
                    )
                    if (
                        x < 0
                        or y < 0
                        or x >= self.output_res[1]
                        or y >= self.output_res[0]
                    ):
                        continue
...
if __name__ == "__main__":
    ...
    training_set = Dataset2D(
        cfg=cfg, set="val", mode="KeypointDetect"
    )  # , cameras_to_use = ['Camera_T', 'Camera_B'])
